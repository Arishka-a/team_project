#!/usr/bin/env python3
"""
GitHub CI Helper: check PR size, list team, update epic PR
Используется в GitHub Actions с RELEASE_TOKEN
"""
...
def check_pr_size(gh, repo_name: str, pr_number: int):
    ...
    try:
        ...
        print(f"Ошибка: PR #{pr_number} не найден или нет доступа: {e}")
        sys.exit(1)
...
def update_epic_description(gh, repo_name: str, pr_number: int):
    """Обновление описания epic PR с child PR"""
    print("\n" + "=" * 60)
    ...
    try:
        ...
        for pr in pr_list:
            ...
            state_text = "merged" if pr.merged else pr.state

            lines.append(
                f"- [{status}] [#{pr.number} - {pr.title}]({pr.html_url}) - `{state_text}`"
            )
            print(f" {status} #{pr.number}: {pr.title}")
            found = True

        if not found:
            lines.append("_Нет связанных PR_")
